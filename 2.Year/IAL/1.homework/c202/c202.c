...
*/
	//s = malloc( sizeof(*s) );	//allokujem zasobnik
	//s->arr[STACK_SIZE]='\0';							//
	s->top=-1;
	if( s == NULL ) {
...
*/
	...
	/*if (s->top == -1)					// ak sa vrchol rovna -1 vraciam true
		return 1;
	else								//inak vraciam false
		return 0;*/
	return s->top == -1 ? 1 : 0;
	//  solved = 0;                      /* V případě řešení, smažte tento řádek! */
}
...
*/
	...
	if (s->top >= (STACK_SIZE-1))	//ak sa vrchol rovna velkosti zasobnika vraciam true
		return 1;
	else 							//inac vraciam false
		return 0;
...
*/	
	  ...
	  if (stackEmpty(s)==0) 	//ak je zasobnik prazdny, hlasim chybu
	  	*c = s->arr[s->top];				// inac zapisem najvyssi znak	
	  else 	
	  {				
	  	stackError(SERR_TOP);
		return;
...
*/
	...
	if (stackEmpty(s) == 0)	//ak je zasobnik prazdny, hlasim chybu	
	{									
		//s->arr[s->top] = '\0';			//najvyssi prvok pola
		s->top -=1;						// inac dekrementujem top a vymazem
	}	
	else 								
		stackError(SERR_TOP);
	//  solved = 0;                      /* V případě řešení, smažte tento řádek! */
...
** funkci stackFull.
*/
	if (stackFull(s)==0)	//ak je zasobnik prazdny, hlasim chybu
	{									
		s->top +=1;
	  	s->arr[s->top] = c;				// a do pola zapisem znak
	}	
		
	else 								// inac inkrementujem vrchol zasobnika
