...
void phong_onInit(int32_t width,int32_t height){
  ...
  /// Zde byste měli vytvořit buffery na GPU,
  
  ///This variable contains buffer with vertex positions.
  
  
  cpu_createBuffers(phong.gpu , 1, &phong.ide1);
  cpu_createBuffers(phong.gpu, 1, &phong.ide2);

  /// nahrát data do bufferů,
  cpu_bufferData( phong.gpu, phong.ide1, sizeof(BunnyVertex)*1048, bunnyVertices);
  cpu_bufferData(phong.gpu, phong.ide2, sizeof(VertexIndex)*3*2092 , bunnyIndices);
 /////////////////////////////////
  cpu_createVertexPullers( phong.gpu, 1, &phong.IdePull);
  ///////////////////////////////// 
  cpu_setVertexPullerHead(phong.gpu, phong.IdePull, 0, phong.ide1, sizeof(float) * 0, sizeof(float) * 6);
  cpu_setVertexPullerHead(phong.gpu, phong.IdePull, 1, phong.ide1, sizeof(float) * 3, sizeof(float) * 6);
  /////////////////////////////////
  cpu_enableVertexPullerHead( phong.gpu, phong.IdePull, 0);
  cpu_enableVertexPullerHead(phong.gpu, phong.IdePull, 1);
  /////////////////////////////////
  phong.IdePr = cpu_createProgram(phong.gpu);
  cpu_attachVertexShader(phong.gpu, phong.IdePr, phong_vertexShader);
  cpu_attachFragmentShader( phong.gpu, phong.IdePr, phong_fragmentShader);
  /////////////////////////////////
  cpu_setAttributeInterpolation(phong.gpu, phong.IdePr, 0, ATTRIB_VEC3, SMOOTH);
  cpu_setAttributeInterpolation(phong.gpu, phong.IdePr, 1, ATTRIB_VEC3, SMOOTH);
  /////////////////////////////////
  // Také byste zde měli zarezervovat unifromní proměnné pro
  //matice
  ...
  cpu_setIndexing(phong.gpu, phong.IdePull, phong.ide2, 2);

  
  ///a další vaše proměnné.
...
void phong_onDraw(SDL_Surface*surface){
  ...
  cpu_useProgram(phong.gpu, phong.IdePr); //usenutie cpu
  cpu_bindVertexPuller(phong.gpu, phong.IdePull);
  UniformLocation const projMatrixUnif = getUniformLocation(phong.gpu, "projectionMatrix"); //do projMatrixUnif ulozim result funkcie getUniformLocation
  UniformLocation const viewMatrixUnif = getUniformLocation(phong.gpu, "viewMatrix");//do viewMatrixUnif ulozim result funkcie getUniformLocation
  cpu_uniformMatrix4fv(phong.gpu, viewMatrixUnif, (float*)&viewMatrix);
  cpu_uniformMatrix4fv(phong.gpu, projMatrixUnif, (float*)&projectionMatrix);
  UniformLocation const camPosUnif = getUniformLocation(phong.gpu, "cameraPosition");
  UniformLocation const lightPosUnif = getUniformLocation(phong.gpu, "lightPosition");
  cpu_uniform3f(phong.gpu, lightPosUnif, phong.lightPosition.data[0], phong.lightPosition.data[1], phong.lightPosition.data[2]);
  cpu_uniform3f(phong.gpu, camPosUnif, cameraPosition.data[0], cameraPosition.data[1], cameraPosition.data[2]);
